...
class MudConsole(OLD_CONSOLE):
    ...
    def _render_buffer(self, buffer: Iterable[Segment]) -> str:
        ...
        for text, style, control in buffer:
            print(f"Text: {text} - {type(text)}, Style: {style} - {type(style)}, Control: {control} - {type(control)}")
            if style:
                if isinstance(style, str):
                    style = self._thread_locals.theme_stack.get(style)
                append(
                    style.render(
                        text,
...
class Stripper:
    def __init__(self):
        self.console = MudConsole(color_system=None, width=99999999999, file=self, record=True)
...
def install_mudrich():
    ...
    print(type(default_styles.DEFAULT_STYLES["table.title"]))

    # This little addition should make ANSIString printable.
    from evennia.utils.ansi import ANSIString
    ANSIString.__rich_console__ = AnsiStringConsole
